...
#version2.0
#Add a colnames to distinguish clouds due to bad channels
# in development
# print doFITS.weighted_avg_and_std

# Version 6.0
# record LBV range for each cloud
...
class MWISPDBSCAN( object ):
    ...
    cleanTBWithDis = None #used only by matchDisTB, to save the distance table with distance assigned

    rawCOFITSConf = "rawCOFITS"
    rmsFITSConf = "rmsFITS"
    dbscanPathConf = "dbscanPath"
    lvPathConf = "lvPath"
    maskPathConf = "maskPath"
    extraTagConf = "extraTag"
    ...
    coreFITS = None
    maskFirst= False
    ################# DBSCAN parameters #### should be fixed
    ...
    doClean=True #### not always

    saveIntFITSname = None
    savePureIntFITSname = None
    saveMaskFITSname =  None
    saveForegroundFITSname = None
    cropExtend=2 #in degree

    regionName = None
    intFolderName = "cloudInt"
    specFolderName = "specAvg"



    areaThresh=0.01 #deg square,

    configFile = "/home/qzyan/WORK/dataMWISP/project/distanceMC/BEEPII/dbscanRecords.ini" ### by default, local saving
    #this file keep record DBSCASN that have been processed, you can alter this file

    #configFile = "/home/qzyan/WORK/dataMWISP/project/distanceMC/BEEPII/regions.ini" ### by default, local saving, if the save name
    #is
    config = None
    ...
    controlMaskFITS=None #and additional mask file
    saveCloudID=None
    saveCloudIndex=None
    #### in order to speed up

    cloudCubeSaveFolder = "cloudSubCubes"

    def __init__(self,regionName=None,configureFile=None ):
        # self.getModelTB()
        print("Running DBSCAN with version ", self.__version__)
        if configureFile is not None and os.path.isfile(configureFile):

            print("Configure file found!")
            self.configFile = configureFile

        if regionName is not None:

            self.getDBSCANcase(regionName )
        pass

    #def initDBSCAN(self,rawCOFITS,averageRMS=None,rmsFITS=None,processPath =None):
    ...
    def setControlMaskFITS(self,controlMaskFITS):
        self.controlMaskFITS=controlMaskFITS
    ...
    def setCatalogSelectionCriteria(self, minVox=None, minChannel=None, hasBeam=None, minPeakSigma=None):
        if minVox is not None:
            self.minVox = minVox

        if minChannel is not None:
            self.minChannel = minChannel
        if hasBeam is not None:
            self.hasBeam = hasBeam
        if minPeakSigma is not None:
    ...
    def setOutputPath(self, targetPath):
        ...
        doFITS.checkPath(self.processPath )

        self.getOutputNames()### redirect the outpyt path
    ...
    def writeConfig(self,configFile=None):
        ...
        self.config[self.regionName] = {}
        self.config[self.regionName] [self.rawCOFITSConf ] = self.rawCOFITS
        self.config[self.regionName] [self.rmsFITSConf ] = self.rmsFITS
        self.config[self.regionName] [self.dbscanPathConf ] = self.processPath
        self.config[self.regionName][self.extraTagConf ] = self.extraTag #### bug fixed#qzyan 2023/10/31


        print("Writting config of ",self.regionName, " to " , self.configFile)
        with open(self.configFile, 'w') as cf:

            self.config.write(cf)


    def set_cloud(self,cloudID):

        self.getCloudIntName(cloudID)
    def set_region(self,regionName):
        """
        ...
        if self.regionName not in self.config.sections():

            print("No configuration of {} found...please add the configure...{}".format(self.regionName,self.configFile ))


        else:

            self.rmsFITS = self.config[self.regionName][self.rmsFITSConf]

            self.rawCOFITS = self.config[self.regionName][self.rawCOFITSConf]
            self.extraTag = self.config[self.regionName][self.extraTagConf ]

            if not  os.path.isfile(self.rawCOFITS):
                print("The CO raw file does not exist for region {}, stop reading configure files.".format( self.regionName) )


                return

            if self.config[self.regionName][self.dbscanPathConf] != "" :

                self.setOutputPath( self.config[self.regionName][self.dbscanPathConf] )

            else:
                defaultPath = os.path.join(self.defaultDBSCANpath,self.regionName)
                self.setOutputPath( defaultPath   ) #using default path
            ...
            self.getCloudIntPath() #using defaults
    ...
    def getOutputNames(self):
        ...
        print("Initializing {}....".format(self.regionName))
        if self.rawCOFITS is None:
            print("CO FITS is needed")
            return

        if not os.path.isfile(self.rawCOFITS):
            print("The raw COFITS does not exist!")
            return

        if self.averageRMS is None and self.rmsFITS is None:
            print("Please set the average RMS or produce the rmsFITS!")

            return



        if self.rmsFITS is not None and  not os.path.isfile(self.rmsFITS):
            print("The rms FITS does not exist!")

            return

        self.rawCOFITS=os.path.abspath(self.rawCOFITS)
        if self.rmsFITS is not None:
            self.rmsFITS=os.path.abspath(self.rmsFITS)


        self.processPath=os.path.abspath(self.processPath)
        doFITS.checkPath(self.processPath)
        ...
        if self.regionName is None:
            self.regionName =   baseName[0:-5] + saveTag  # used to save configures
        ...
        self.noiseFITSname = preFix + "_noise.fits"
    ...
    def getInfo(self):
        """
        basis information from header
        :return:
        """
        dataCO,headCO= doFITS.readFITS(self.rawCOFITS) #### at this stage rawCOFITS is


        self.pixelSize=headCO["CDELT2"]*60#in armin
        self.velResolution = headCO["CDELT3"]/1000. #in km/s
    ...
    def getDBSCANTag(self):
        ...
        saveName = "dbscanS{}P{}Con{}".format(self.cutoff_sigma, self.minPts, self.connectivity)+self.extraTag
    ...
    def computeDBSCAN(self, splitN=1):
        ...
        if self.controlMaskFITS is not None:
            print("Masking data cube with extra mask file ",self.controlMaskFITS)
            forceMask,headForeMask = doFITS.readFITS( self.controlMaskFITS )
            COdata[forceMask==0]=0
            del forceMask
            gc.collect()
        ...
        if self.maskFirst:
            ...
            extraMask = self.getIsolatedMask(splitN=splitN)

            goodValues =  extraMask  # np.logical_and(goodValues,)
    ...
    def getExtend(self,dataCube):
        """

        :param dataCube:
        :return:
        """
        subNz, subNy, subNx = dataCube.shape
        subExtend = np.zeros([subNz + 2, subNy + 2, subNx + 2], dtype=np.int32)
        subExtend[1:-1, 1:-1, 1:-1] = dataCube
        return subExtend
    ...
    def getIndices(self, Z0, Y0, X0, values1D, choseID,pixN=None ):
        if pixN is not None:


            #np.searchsorted(values1D,choseID)  #np.where(values1D == choseID)
            endIndex =  pixN

            cX0 = X0[0:endIndex ]  # X0[cloudIndices]
            cY0 = Y0[0:endIndex  ]  # Y0[cloudIndices]
            cZ0 = Z0[0:endIndex  ]  # Z0[cloudIndices]




            return tuple([cZ0, cY0, cX0]) , values1D[endIndex+1:], Z0[endIndex:],Y0[endIndex:],X0[endIndex:]
        else:
    ...
    def getEmptyCat(self, getEmptyRow=False):
        ...
        newTB = Table(names=("_idx" ,), dtype=('i8', )  )



        ###
        newTB.add_column(  Column( name="area_exact", dtype='f8',unit= u.arcmin*u.arcmin  )  )
        #####

        newTB.add_column( Column(  name="v_cen", dtype='f8',unit= u.km/u.s  )  )
        ####


        newTB.add_column( Column( name="v_rms", dtype='f8',unit= u.km/u.s  )  )
        ####


        newTB.add_column(Column( name="x_cen", dtype='f8',unit= u.deg  )  )


        ####
        newTB.add_column( Column( name="y_cen", dtype='f8',unit= u.deg  )  )
        ####


        newTB.add_column( Column( name="l_rms", dtype='f8',unit= u.deg  )  )


        ####
        newTB.add_column( Column( name="b_rms", dtype='f8',unit= u.deg  )  )
        newTB.add_column( Column( name="pixN", dtype='i8'  )  )
        newTB.add_column( Column( name="area_accurate", dtype='f8',unit=   u.arcmin*u.arcmin    )  )



        ####
        newTB.add_column( Column( name="peak", dtype='f8',unit= u.Kelvin  )  )


        ####
        newTB.add_column( Column( name="peakB", dtype='i8'   )  )
        ####
        newTB.add_column( Column(name="peakL", dtype='i8'   )  )
        ####
        newTB.add_column( Column( name="peakV", dtype='i8'  )  )


        ####LBV of peak
        newTB.add_column( Column( name="peakBworld", dtype='f8'   )  )
        ####
        newTB.add_column( Column(name="peakLworld", dtype='f8'   )  )
        ####
        newTB.add_column( Column( name="peakVworld", dtype='f8'  )  )
        ...
        newTB.add_column( Column( name="sum", dtype='f8',unit= u.Kelvin  )  )
        ####

        newTB.add_column( Column( name="flux", dtype='f8',unit= u.Kelvin* u.km/u.s *u.arcmin*u.arcmin   )  )


        ####
        newTB.add_column( Column( name="lineWidth", dtype='f8',unit= u.km/u.s )  )

        ####
        newTB.add_column(Column(  name="meanSNR", dtype='f8'))
        newTB.add_column(Column(  name="meanT", dtype='f8'))




        newTB.add_column( Column( name="allChannel", dtype='i8' )  )




        #########
        startLindexCol="startLindex"
        startBindexCol="startBindex"
        startVindexCol="startVindex"

        newTB.add_column(Column(  name= startLindexCol, dtype='f8'))
        newTB.add_column(Column(  name= startBindexCol, dtype='f8'))
        newTB.add_column(Column(  name= startVindexCol, dtype='f8'))




        endLindexCol="endLindex"
        endBindexCol="endBindex"
        endVindexCol="endVindex"
        newTB.add_column(Column(  name= endLindexCol, dtype='f8'))
        newTB.add_column(Column(  name= endBindexCol, dtype='f8'))
        newTB.add_column(Column(  name= endVindexCol, dtype='f8'))


        startLCol="startL"
        startBCol="startB"
        startVCol="startV"
        newTB.add_column(Column(  name= startLCol, dtype='f8'))
        newTB.add_column(Column(  name= startBCol, dtype='f8'))
        newTB.add_column(Column(  name= startVCol, dtype='f8'))

        endLCol="endL"
        endBCol="endB"
        endVCol="endV"

        newTB.add_column(Column(  name= endLCol, dtype='f8'))
        newTB.add_column(Column(  name= endBCol, dtype='f8'))
        newTB.add_column(Column(  name= endVCol, dtype='f8'))



        #### maximum and minimum Z of cloud, exact Index
        newTB.add_column( Column( name="minZ", dtype='i8' )  )
        newTB.add_column( Column( name="maxZ", dtype='i8' )  )

        ####

        newTB.add_column( Column( name="has22", dtype='i4' )  )
        ####



        newTB.add_column( Column( name="touchLB", dtype='i4' )  )
        newTB.add_column( Column( name="touchL", dtype='i4' )  )
        newTB.add_column( Column( name="touchB", dtype='i4' )  )
        newTB.add_column( Column( name="touchV", dtype='i4' )  )
        newTB.add_column( Column( name="complete", dtype='i4' )  )
    ...
    def getDBSCANCatlog(self, doClean=True):
        ...
        sizeV = headCluster["CDELT3"] /1000. #km/s
        ...
        if self.rmsFITS is not None:
            ...
            if len(rmsData.shape) ==3 and rmsData.shape[0]==1 : #### the first cooridiane is wrong
                self.rmsData=rmsData[0]
        ...
        if 1: #### sort index
            sortIndices = np.argsort(clusterValue1D)
            iZ, iY, iX = clusterIndex1D
            iZ_S = iZ[sortIndices]
            iY_S = iY[sortIndices]
            iX_S = iX[sortIndices]
            clusterValue1D = clusterValue1D[sortIndices]
            clusterIndex1D = (iZ_S, iY_S, iX_S)
        ...
        #print( "{} DBSCAN clusters found".format( len(ids) ) )
        ...
        if doClean:
            ...
            checkN = len(GoodIDs)
            print(checkN, "cluster has more than {} voxels.".format(self.minVox))

        allCounts = np.repeat(count,count)
        if len(allCounts) !=len(clusterValue1D):
            print(  len(allCounts),len(clusterValue1D),"should be equal???Terminating ..... " )
            aaaaaaaaa
        ...
        if 1: ####  remove useless indecies
            selectGoodIndices  =  allCounts>=  self.minVox
            iZ, iY, iX = clusterIndex1D 
            iZ_S = iZ[selectGoodIndices]
            iY_S = iY[selectGoodIndices]
            iX_S = iX[selectGoodIndices]
            clusterValue1D = clusterValue1D[selectGoodIndices]
            clusterIndex1D = (iZ_S, iY_S, iX_S)



            ### remove those clouds with peak less that  self.minPeakSigma

            ### getCOvalues
            coValues1D = dataCO[clusterIndex1D]
            rmsValues1D = self.rmsData[(iY_S,iX_S)]

            SNRValues1D = coValues1D/rmsValues1D




        emptyTB = self.getEmptyCat( )
        emptyTB.add_row()


        if 1: #sort indices

            clusterValue1DSearch =  clusterValue1D.copy()

            clusterIndex1DSearch =  (clusterIndex1D[0].copy(),clusterIndex1D[1].copy(),clusterIndex1D[2].copy()  )


            Z0Search, Y0Search, X0Search = clusterIndex1DSearch
            SNRValues1DSearch = SNRValues1D.copy()
        while 1 :
            copyTB = emptyTB.copy()
            emptyTB= vstack((copyTB,emptyTB))
            if len(emptyTB)> checkN:
                break
        ...
        for i in range(len(GoodIDs)):
            ...
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! becare full
            cloudIndex ,clusterValue1DSearch , Z0Search,Y0Search,X0Search= self.getIndices(Z0Search, Y0Search, X0Search, clusterValue1DSearch, newID, pixN= pixN     )

            cloudMaxSNR = np.max(SNRValues1DSearch[0:pixN])

            if cloudMaxSNR<self.minPeakSigma:
                SNRValues1DSearch = SNRValues1DSearch[pixN:]
                #print("Reject ",newID, " Due to low peak?")
                continue
            else:
                SNRValues1DSearch = SNRValues1DSearch[pixN:]

            #print(len(clusterValue1DSearch),"lenght of search?",newID)

            #SNRValues1DSearch = SNRValues1DSearch[pixN:]
            #### clustering ID are changing ...

            #record LBV range

            #### any fast way to get area ?

            #pbar.update(i)
            #continue  #### for the tes of time


            cloudV = cloudIndex[0]
            diffVs = np.unique(cloudV)
            if len(diffVs) < self.minChannel and doClean:  # reject all cloud that has channels less than 3 channels
                #print("Reject ",newID, " Due to narrow velocity??")
                continue
            ...
            newRow[ startVindexCol ] =  min(cloudV)
            newRow[ endVindexCol ] =  max(cloudV)

            newRow[ startBindexCol ] =  min(cloudB)
            newRow[ endBindexCol ] =  max(cloudB)

            newRow[ startLindexCol ] =  min(cloudL)
            newRow[ endLindexCol ] =  max(cloudL)

            l0,b0,v0=wcsCloud.wcs_pix2world(newRow[ endLindexCol ],newRow[ startBindexCol ] ,newRow[ startVindexCol ]  , 0   )
            newRow[startLCol] = l0
            newRow[startBCol] = b0
            newRow[startVCol] = v0/1000.



            l1,b1,v1=wcsCloud.wcs_pix2world(newRow[ startLindexCol ],newRow[ endBindexCol ] ,newRow[ endVindexCol ]  , 0   )
            newRow[endLCol] = l1
            newRow[endBCol] = b1
            newRow[endVCol] = v1/1000.
            ...
            if peak < peakSimga and doClean:  # accurate to lines, what is wrong here?......
                #nothing wrong is here, peak SNR may be larger than 5 , but at the peak position, the SNR may be less than 5 sigma
                #very strange ....
            ...
            newRow["flux"] = sumCO * pixelArea * sizeV ####
            newRow["sum"] = sumCO

            newRow["x_cen"], newRow["y_cen"], newRow["v_cen"] = wcsCloud.wcs_pix2world(Lcen, Bcen, Vcen, 0)
            newRow["v_cen"] = newRow["v_cen"] / 1000.

            newRow["peakLworld"], newRow["peakBworld"], newRow["peakVworld"] = wcsCloud.wcs_pix2world(peakL, peakB, peakV, 0)
            newRow["peakVworld"] = newRow["peakVworld"] / 1000.
            ...
            ####
            rawCrop = dataCO[:, minY:maxY + 1, minX:maxX + 1]
            zeroprojMask = zeroProjection[minY:maxY + 1, minX:maxX + 1]

            rawCrop =  rawCrop*zeroprojMask


            averageSpectraRaw = np.nansum(rawCrop, axis=(1, 2))/projectionPixN
            #### find the width of linewidth based on real spectra? #### should we save this spectra? then investigate them

            if self.saveSpec:

                saveName= self.getSpecSaveName(newID)

                np.save(saveName,averageSpectraRaw)
            cleanCrop = dataZero[:, minY:maxY + 1, minX:maxX + 1]
            averageSpectraClean = np.nansum(cleanCrop, axis=(1, 2))

            # count the number spectra

            totalSpectral = projectionPixN / 1.

            meanSpectral = averageSpectraClean / 1. / totalSpectral
            ...
            #### record to the sake of clean
            ##### record indices here
            idRecord.append(newID)
            indicesRecord.append(cloudIndex) 
        ...
        pbar.finish()
        # save the clouds
        emptyTB = emptyTB[emptyTB["area_exact"]>0]
        ...
        if doClean:
            ...
            print("{} clouds found by DBSCAN!!!!".format(len(emptyTB)))
            emptyTB.write(self.cleanCatName, overwrite=True)

            #### what do we know
            self.saveCloudID = idRecord
            self.saveCloudIndex = indicesRecord #
        else:
            emptyTB.write(self.rawCatName, overwrite=True)

    def getSpecSaveName(self,cloudID):

        saveName = "SpecRaw{}.npy".format(cloudID)
        saveName = os.path.join(self.cloudSpecPath, saveName)
        ####
        return saveName
    ...
    def getCloudIntPath(self  ):
        ...
        #print(self.processPath,"??????????????????????????")
        #doFITS.checkPath(self.processPath)
        if self.cloudIntPath is None:


            self.cloudIntPath =  os.path.join(self.processPath, self.intFolderName )
            doFITS.checkPath(self.cloudIntPath)

            ##########
            self.cloudSpecPath =  os.path.join(self.processPath, self.specFolderName )
            doFITS.checkPath(self.cloudSpecPath)
        ####

        else:
            pass

            # the output path has been assigned directely





    def produceCloudIntFITS(self,  inputTB=None, outputPath=None, pureInt=False, foreground=False, complete=True, areaControl=True, localVindx=None, onID=None,  doCrop=True ,cropExtend=None):
        """
        ...
        if localVindx is not None and localVindx!="auto":
        ...
        if complete: #### usually this is true
            TB = TB[TB["complete"] > 0]  # only complete ones

        TB.sort("area_exact")
        TB.reverse()
        if areaControl:# too many or not, it depends
            ###  remove small molecular clouds, other wise there are too many
            TB = TB[TB["area_exact"] >= self.areaThresh * 3600]  ## Remove small molecular clouds


            print("After removing smaller molecular clouds, {} left".format(len(TB) ) )

        else:
            print("All complete molecular clouds, {} left".format(len(TB) ) )
        ...
        WCSco = WCS(headCluster)
        ####
        if localVindx == "auto":  # calculate local vindex automatically
            _,_,localV=WCSco.wcs_world2pix(180,0,0,0)
            self.localVindex= int(  np.round(localV) )
            self.localVindex=max([0,self.localVindex ])
            self.localVindex=min([Nz-1,self.localVindex ])


            
            #### set localVindex, at the index of 0 km/s
        ...
        for eachDBRow in TB:
            ...
            if pureInt:  # usually do no use this
                ...
                #print(self.savePureIntFITSname,"What is this?")

                fits.writeto(self.savePureIntFITSname, sumCOPure, header=headCluster, overwrite=True)
                #print("Why this does not work")
            ...
            if foreground:  # the way of generating foreground fits is different, the following code is used to produce foreground fits for Q2
                ...
                if abs(endVindex-self.localVindex) < abs(startV0-self.localVindex):
                    ...
                    #print(starForeI,endForeI,"Fore range",startV0,endVindex,"cloud range")
            ...
            if doCrop: #crop the fits
                ...
                if os.path.isfile(self.savePureIntFITSname):
                ...
                if os.path.isfile(self.saveForegroundFITSname):
    ...
    def produceCleanFITS(self):
        ...
        #
        if self.saveCloudID is not None and self.saveCloudIndex is not None:
            print("Using saved index to speed up!")
        ...
        if self.saveCloudID is not None and self.saveCloudIndex is not None:
            ...
            for i in range(totalN):
                cloudIndex=self.saveCloudIndex[i]
                cloudID = self.saveCloudID[i ]
        ...
        dataCO,headCO=doFITS.readFITS(self.rawCOFITS)
        if len( dataCO.shape )==4:
            dataCO = dataCO[0]
        dataCO[ np.where(returnCluster>0) ]=np.nan

        if self.writeNoise:


            fits.writeto(self.noiseFITSname, dataCO, header=headCluster, overwrite=True)
    ...
    def getLBVrangeOfClouds(self):
        """
        A function of get the LBV range of clouds
        """


        cloudTB=Table.read(self.cleanCatName)
        catNameWithLBVrange = doFITS.addSuffix(self.cleanCatName,"_WithLBVRange")

        dataCluster, headCluster = myFITS.readFITS(self.cleanLabelFITSName)


        minV = np.nanmin(dataCluster[0])
        wcsCloud = WCS(headCluster, naxis=3) ### usd to convert index to lbv
        clusterIndex1D = np.where(dataCluster > minV)
        clusterValue1D = dataCluster[clusterIndex1D]
        Z0, Y0, X0 = clusterIndex1D
        ...
        startBindexCol="startBindex"
        startVindexCol="startVindex"
        cloudTB[startLindexCol]= cloudTB["peakL"]*0
        cloudTB[startBindexCol]= cloudTB["peakL"]*0
        cloudTB[startVindexCol]= cloudTB["peakL"]*0
        ...
        endBindexCol="endBindex"
        endVindexCol="endVindex"
        cloudTB[endLindexCol] = cloudTB["peakL"] * 0
        cloudTB[endBindexCol] = cloudTB["peakL"] * 0
        cloudTB[endVindexCol] = cloudTB["peakL"] * 0

        startLCol="startL"
        startBCol="startB"
        startVCol="startV"
        cloudTB[startLCol] = cloudTB["x_cen"] * 0
        cloudTB[startBCol] = cloudTB["y_cen"] * 0
        cloudTB[startVCol] = cloudTB["v_cen"] * 0
        ...
        endBCol="endB"
        endVCol="endV"
        cloudTB[endLCol] = cloudTB["x_cen"] * 0
        cloudTB[endBCol] = cloudTB["y_cen"] * 0
        cloudTB[endVCol] = cloudTB["v_cen"] * 0
        ...
        for eachC in cloudTB:
            ...
            eachC[ startVindexCol ] =  min(cloudZ0)
            eachC[ endVindexCol ] =  max(cloudZ0)

            eachC[ startBindexCol ] =  min(cloudY0)
            eachC[ endBindexCol ] =  max(cloudY0)

            eachC[ startLindexCol ] =  min(cloudX0)
            eachC[ endLindexCol ] =  max(cloudX0)

            l0,b0,v0=wcsCloud.wcs_pix2world(eachC[ endLindexCol ],eachC[ startBindexCol ] ,eachC[ startVindexCol ]  , 0   )
            eachC[startLCol] = l0
            eachC[startBCol] = b0
            eachC[startVCol] = v0/1000.



            l1,b1,v1=wcsCloud.wcs_pix2world(eachC[ startLindexCol ],eachC[ endBindexCol ] ,eachC[ endVindexCol ]  , 0   )
            eachC[endLCol] = l1
            eachC[endBCol] = b1
            eachC[endVCol] = v1/1000.


        cloudTB.write(catNameWithLBVrange,overwrite=True)

    def getCloudCubeFITS(self, cloudID, savePath=None):
        """

        """
        if savePath is None:
            savePath = os.path.join(self.processPath,  self.cloudCubeSaveFolder  )

        saveName = "cloud{}cube_{}.fits".format(cloudID ,self.extraTag)

        return os.path.join(savePath, saveName  )


    def produceIndividualClouds(self,  savePath=None, noiseMask=0,    inputTB=None,cropNoise=False ):
        """
        ...
        if inputTB is not None  :
            ...
            print("Removing clouds with area less than {} deg".format(self.areaThresh))
        print("Producing Individual clouds for {} clouds".format(len(cloudTB)))
        ...
        #yxProj=np.zeros_like(dataCO[0,:,:],dtype=np.int64 )
        #zxProj=np.zeros_like(dataCO[:,0,:],dtype=np.int64 )
        #zyProj=np.zeros_like(dataCO[:,:,0],dtype=np.int64 )
        ...
        for eachC in cloudTB:
            ...
            cloudID = eachC["_idx"]
            saveName =  self.getCloudCubeFITS(cloudID,savePath=savePath) #"cloud{}cube.fits".format(cloudID)
            ...
            fits.writeto(saveName , cropData, header=cropWCS.to_header(), overwrite=True)
    ...
    def pipeLine(self, redoDBSCAN=True, onlyCat=False, writeconfig=False, splitN=1,getIntFITS=False,getCubeFITS=False):
        """
        :param rawCOFITS:
        :param redoDBSCAN:
        :param outPath:
        :param rmsFITS:
        :param rmsMean:
        :param splitN: to overcome the memory problem
        :param getIntFITS:
        :param getCubeFITS:
        :return:
        """
        ...
        if writeconfig:
            self.writeConfig()
        #do we need to write down the case?

        if redoDBSCAN:
            print('Redo DBSCAN')
            self.computeDBSCAN(splitN= splitN )
            self.getDBSCANCatlog(doClean=self.doClean)
        ...
        if not redoDBSCAN and not onlyCat:
            print('Recalling old DBSCAN')
    ...
    def setConfigFile(self,configFile):
        ...
        #else
        pathConfigure,fileConfigure=os.path.split(configFile)
        if os.path.isdir(pathConfigure):
            ####create the files
    ...
    def getDBSCANcase(self,regionName,configFile=None ):

        self.setConfigFile(configFile)
        self.set_region(regionName)
...
doDBSCAN.setConfigFile(None)
doDBSCAN.extraTag=""
doDBSCAN.rawCOFITS=""
doDBSCAN.rmsFITS=""
doDBSCAN.pipeLine(writeconfig=True )

"""


### recall DBSCAN results
